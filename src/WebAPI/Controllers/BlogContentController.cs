...
{
    [HttpGet("{id:int}")]
    public async Task<BlogContentDto> GetById(int id, CancellationToken cancellationToken = default)
        => await Mediator.Send(new GetBlogContentByIdQuery(id), cancellationToken);

    [HttpGet("{id:int}/detail")]
    public async Task<BlogContentWithTextAndMetasDto> GetWithTextById(int id, CancellationToken cancellationToken = default)
    {
        return await Mediator.Send(new GetBlogContentWithTextByIdQuery(id), cancellationToken);
    }

    [HttpGet]
    public async Task<PaginatedList<BlogContentDto>> GetBlogContents([FromQuery] GetBlogContentWithPaginationQuery query, CancellationToken cancellationToken = default)
    {
